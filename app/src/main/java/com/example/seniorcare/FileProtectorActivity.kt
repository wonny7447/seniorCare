...
class FileProtectorActivity : AppCompatActivity() {
    ...
    var relationUid : String ?= null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_file_protector)

        relationUid = intent.getStringExtra("relationUid").toString()
        ...
        val listRef = storage.reference.child(relationUid!!)
    ...
    private fun uploadFile(attachUri: Uri, attachUriString: String) {
        ...
        // 사용자의 uid 경로에 파일 업로드
        var storageRef = FirebaseStorage.getInstance().reference.child(relationUid!!).child(fileName)
